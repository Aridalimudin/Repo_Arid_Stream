...
object AridDrakorExtractor : AridDrakor() {
    ...
    private const val KK_TAG = "AridDrakorKK"
    ...
    // ditangani TMDB di AridDrakor.
    //
    // Alur: TMDB load() -> LinkData -> loadLinks() -> invokeMoviebox()
    //       -> search/v2 -> season-info -> play-info -> ExtractorLink (+ Cookie)
    //
    // DIAGNOSTIK: filter logcat dengan tag "AridDrakorMB".
    private const val MB_TAG = "AridDrakorMB"

    /**
     * Dipanggil dari AridDrakorPlugin.load(). Menyiapkan identity persisten
        ...
        finalStreams.forEach { stream ->
            ...
            callback.invoke(
                newExtractorLink("MovieBox", label, streamUrl, INFER_TYPE) {
                    ...
                    // Cookie di sini dibaca lagi oleh AridDrakor.getVideoInterceptor()
    ...
    private object MovieboxHelper {
        ...
        //   sini : SharedPreferences("AridDrakor_identity")["apkdeviceid"]
        //
        ...
        // ---------------------------------------------------------------
        private const val ID_PREFS = "AridDrakor_identity"
