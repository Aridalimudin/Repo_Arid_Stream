...
object AridFilmSemiExtractor : AridFilmSemi() {
    ...
    private const val KK_TAG = "AridFilmSemiKK"
    ...
    // ditangani TMDB di AridFilmSemi.
    //
    // Alur: TMDB load() -> LinkData -> loadLinks() -> invokeMoviebox()
    //       -> search/v2 -> season-info -> play-info -> ExtractorLink (+ Cookie)
    //
    // DIAGNOSTIK: filter logcat dengan tag "AridFilmSemiMB".
    private const val MB_TAG = "AridFilmSemiMB"

    /**
     * Dipanggil dari AridFilmSemiPlugin.load(). Menyiapkan identity persisten
        ...
        finalStreams.forEach { stream ->
            ...
            callback.invoke(
                newExtractorLink("MovieBox", label, streamUrl, INFER_TYPE) {
                    ...
                    // Cookie di sini dibaca lagi oleh AridFilmSemi.getVideoInterceptor()
    ...
    private object MovieboxHelper {
        ...
        //   sini : SharedPreferences("AridFilmSemi_identity")["apkdeviceid"]
        //
        ...
        // ---------------------------------------------------------------
        private const val ID_PREFS = "AridFilmSemi_identity"
